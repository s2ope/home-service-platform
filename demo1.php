...
$query = "SELECT provider.fname, provider.mname, provider.lname, service_request.srid as srid, 
...
$stmt->execute();
$stmt->store_result();
$stmt->bind_result($fname, $mname, $lname, $srid, $address, $city, $state, $country, $phnno, 
...
<body class="is-preload">
    ...
    <div id="serviceModal" class="modal">
        <div class="modal-content">
            ...
            <h2 style="color: #28a745; margin-bottom: 1.5rem;">Service Request Details</h2>
            <div class="service-details">
                <div class="detail"><strong>Provider:</strong> <span id="modal-provider-name"></span></div>
                <div class="detail"><strong>Service Name:</strong> <span id="modal-service-name"></span></div>
                <div class="detail"><strong>Address:</strong> <span id="modal-address"></span></div>
                <div class="detail"><strong>Phone Number:</strong> <span id="modal-phone"></span></div>
                <div class="detail"><strong>Request Date & Time:</strong> <span id="modal-date-time"></span></div>
                <div class="detail"><strong>Status:</strong> <span id="modal-status" class="badge"></span></div>
                <div class="detail"><strong>Work Status:</strong> <span id="modal-work-status" class="badge"></span></div>
                <div class="detail"><strong>Payment Status:</strong> <span id="modal-payment-status" class="badge"></span></div>
                <div class="detail"><strong>Amount to Pay:</strong> <span id="modal-amount-to-pay" class="amount"></span></div>
            </div>
        </div>
    </div>
...
<div id="ratingModal" class="modal">
    ...
    <span class="close" onclick="closeRatingModal()">&times;</span> 
        <h2 style="color: #28a745; margin-bottom: 1.5rem;">Rate Your Experience</h2>
    ...
    $workstatus_class = $workstatus == 0 ? "badge-warning" : "badge-success";
                                    ...
                                    $date_time = date('M j, Y', strtotime($fdate)) . ' at ' . date('g:i A', strtotime($rtime));
                                ...
                                ?>
                                    <div class="<?= $card_class ?>" data-search="<?= htmlspecialchars($card_data) ?>">
                                        <?php if ($read_status_c == 0): ?>
                                            <div class="notification-badge">!</div>
                                        <?php endif; ?>
                                        
                                        <div class="card-header">
                                            <h3 class="card-title"><?= htmlspecialchars($full_name) ?></h3>
                                            <strong>
                                            <div class="service-name"><?= htmlspecialchars($sname) ?></div>
                                            </strong>
                                        </div>

                                       <!-- Inside your booking card loop -->
<div class="card-content">
    <div class="card-section">
        <div class="card-label">Request Date & Time</div>
        <div class="card-value"><?= htmlspecialchars($date_time) ?></div>
    </div>

    <div class="card-section">
        <div class="card-label">Amount to Pay</div>
    ...
    <div class="card-section" style="grid-column: span 2;">
        <div style="display: flex; gap: 10px; flex-wrap: wrap;">
            <div>
                <div class="card-label">Request Status</div>
                <span class="badge <?= $status_class ?>"><?= $status_label ?></span>
            </div>
            <div>
                <div class="card-label">Work Status</div>
                <span class="badge <?= $workstatus_class ?>"><?= $workstatus_label ?></span>
            </div>
            <div>
                <div class="card-label">Payment Status</div>
                <span class="badge <?= $paymentstatus_class ?>"><?= $paymentstatus_label ?></span>
            </div>
        </div>
    </div>
</div>
                                        ...
                                        <div class="notification-content">
                                            ...
                                            <div class="notification-message"><?= htmlspecialchars($msgc) ?></div>
                                        </div>
                                        <?php endif; ?>

                                        <div class="button-container">
                                        <?php if ($read_status_c == 0 || $status == 2): ?>
                                            <form method="post" style="margin-right: auto;">
                                                ...
                                                <?php if ($workstatus!=1):?>
                                                <button type="submit" class="btn btn-mark-read primary">
                                                    <i class="fas fa-check-circle" style="margin-right: 5px;"></i> Mark as Read
                                                </button>
                                                <?php endif;?>
                                            </form>
                                        <?php endif; ?>
                                        ...
                                        <?php if ($status == 1 && $payment_status == 1): ?>
                                            <button class="btn btn-pay primary" onclick="openRatingModal('<?= $srid ?>', '<?= $provider_id ?>')">
                                                <i class="fas fa-star" style="margin-right: 5px;"></i> Rate Service
                                            </button>
                                        <?php endif; ?>
   ...
   onclick="openModal(
    ...
    <i class="fas fa-eye" style="margin-right: 5px;"></i> View More
</a>
                                                                                <?php if (!$pay_disabled): ?>
                                                <form id="esewaForm<?= $srid ?>" action="https://rc-epay.esewa.com.np/api/epay/main/v2/form" method="POST">
                                                    ...
                                                    <button type="submit" class="btn btn-pay primary">
                                                        ...
                                                        <img src="Esewa_logo.webp" class="esewa-logo">
                                                    </button>
                                                </form>
    ...
    <script>
        ...
        function openModal(name, service, address, phone, date, time, status, workstatus, payment_status, amount) {
            ...
            if (workstatus == 0) {
                ...
                workStatusBadge.textContent = 'Pending';
            } else {
                workStatusBadge.className = 'badge badge-success';
                workStatusBadge.textContent = 'Completed';
            }
    ...
    // Close service modal if clicked outside
    ...
    function closeRatingModal() {
    document.getElementById('ratingModal').style.display = 'none';
    resetStars(); // Reset the stars when closing
}
    ...
    stars.forEach(star => {
        ...
        star.addEventListener('mouseover', function() {
            ...
            stars.forEach((s, index) => {
                if (index < hoverRating && !s.classList.contains('fas')) {
